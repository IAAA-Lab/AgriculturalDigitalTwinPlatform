<script lang="ts">
	...
	export let pred: any;
	export let idema: string | undefined;
</script>

<section class="card">
	{#await enclosuresService.getHistoricalWeather(idema || '', startDate, new Date())}
		<Loading />
	{:then hw}
		{@const sortedHw = hw?.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())}
		<div class="card-inner" style="width: 100%;">
			<div class="header">
				<i class="fi fi-rr-cloud-showers-heavy" />
				<p class="m-0">Precipitaciones (Últimos 7 días)</p>
			</div>
			<div style="max-height: 250px; min-height: 150px;">
				<Chart
					data={{
						type: 'bar',
						data: {
							...
							labels: sortedHw?.map((h) => h?.date?.split('T')[0])
						},
						options: {
							elements: {
								bar: {
	...
	{/await}
	<div
		style="display: flex; flex-direction: row; justify-content: space-between; align-items: start; column-gap: 0.5rem;"
	>
		<div
			style="display: flex; flex-direction: column; justify-content: center; align-items: center; row-gap: 0.5rem;"
		>
		...
		<div class="card-inner" style="height: 93%; width: 100%">
			<div class="header">
				<i class="fi fi-rr-temperature-high" />
				<p class="m-0">Temperatura</p>
			</div>
			<div style="max-height: 400px; min-height: 150px">
				<Chart
					data={{
						data: {
							datasets: [
