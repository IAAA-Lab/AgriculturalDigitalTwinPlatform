...
async def extract_ccaa_and_province(enclosure_id: str):
    # Connect to MinIO
...
async def join_information(enclosure_geographic_info, crs, enclosure_meteorological_stations, year: int, enclosure_id: str, ccaa: str, province: str):
    joined_enclosure = {
        "id": enclosure_id,
        "year": int(year),
        "type": "Feature",
...
async def load_enclosures(enclosures):
    # Connect to MongoDB
...
async def recintos_etl(year: int, enclosure_ids: list[str]):
    logger = get_run_logger()
    enclosures_geographic_info = await extract_geographic_info(enclosure_ids, year)
    joined_enclosures = []
    for enclosure_geographic_info in enclosures_geographic_info["features"]:
        try:
            ...
            meteorological_info = await extract_meteorological_station(enclosure_id)
            ccaa, province = await extract_ccaa_and_province(enclosure_id)
            ...
            logger.error(f"Error processing enclosure: {enclosure_id} - {e}")
    await load_enclosures(joined_enclosures)
